...
- **ツールとしてbashしか持たないのが最大の特徴。** LLMからの「ツール呼び出し」はすべてbashコマンドという形で表現される。一般的なLLMのツール呼び出し(function calling / tool calling)は、モデルが呼び出したい関数名と引数をJSON形式で返し、それをAPI側の専用の仕組み(OpenAIでいう`tools`パラメータや`tool_calls`)でやり取りするが、このエージェントはそれを一切使わない。応答は普通のテキストで、その中の```bash```コードブロックを正規表現で取り出しているだけ(`parse_action`)。ツールの種類を増やしたい場合もJSON定義を追加するのではなく、単に「bashで実行できるコマンド」を増やすだけで済む
